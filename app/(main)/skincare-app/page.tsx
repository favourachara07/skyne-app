    ...
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-xl shadow-lg">
      ...
      <div className="space-y-6">
            ...
            {["light-medium", "medium", "medium-deep", "deep"].map((tone) => (
              <button
                ...
                onClick={() =>
                  setAnswers((prev) => ({
                    ...prev,
                    melaninTone: tone as MelaninTone,
                  }))
                }
                className={`p-3 rounded-lg border-2 text-sm font-medium capitalize transition-colors ${
        ...
        <button
          ...
          disabled={
            !answers.skinType || !answers.climate || !answers.melaninTone
          }
    ...
    <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-6 rounded-xl border-2 border-dashed border-purple-200">
      <div className="text-center">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-2">
          Optional AI Skin Analysis
        </h3>
        <p className="text-gray-600 mb-4">
          Upload a photo for personalized insights (Coming Soon)
        </p>
        <button
          onClick={handleScan}
          disabled={scanning}
...
function generateRecommendations(
  userProfile: SkincareQuizAnswers,
  season = "default"
) {
  ...
  // Cleanser selection
  const cleanser =
    PRODUCT_DATABASE.cleansers.find(
      (p) =>
  ...
  if (
    ...
    const vitaminC = PRODUCT_DATABASE.treatments.find(
      (p) => p.type === "antioxidant"
    );
    const brightening = PRODUCT_DATABASE.treatments.find(
      (p) => p.type === "brightening"
    );

    if (vitaminC)
      routine.AM.push({ step: "treat", product: vitaminC, order: 2 });
    if (brightening)
      routine.PM.push({ step: "treat", product: brightening, order: 2 });
  }

  // Moisturizer selection
  const moisturizer =
    PRODUCT_DATABASE.moisturizers.find(
      (p) =>
        p.skinTypes.includes(skinType) &&
        p.melaninFriendly &&
  ...
  const sunscreen =
    PRODUCT_DATABASE.sunscreens.find(
      (p) => p.skinTypes.includes(skinType) || p.skinTypes.includes("all")
  ...
  if (season === "dry" && climate === "tropical") {
    ...
    routine.PM.push({
      step: "extra-moisture",
      product:
        PRODUCT_DATABASE.moisturizers.find((p) => p.type === "rich") ||
        PRODUCT_DATABASE.moisturizers[0],
      order: 4,
    });
  }
...
function SeasonalRefresh({
  onRefresh,
}: {
  onRefresh: (season: string) => void;
}) {
...
export default function SkincareApp() {
  ...
  const [userProfile, setUserProfile] = useState<SkincareQuizAnswers | null>(
    null
  );
  interface RoutineStep {
    step: string;
  ...
  const handleAiScanComplete = (results: {
    ...
    if (
      ...
      const updatedProfile = {
        ...
        concerns: [
          ...new Set([
            ...(userProfile.concerns as string[]),
            ...results.detectedConcerns,
          ]),
        ] as Concern[],
      };
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-600">
            Personalized skincare for melanin-rich skin in tropical climates
          </p>
        </div>
            ...
            {aiScanResults && (
              <div className="bg-purple-50 p-6 rounded-xl border border-purple-200">
                ...
                <p className="text-purple-700 mb-2">
                  Confidence: {Math.round(aiScanResults.confidenceScore * 100)}%
                </p>
                <p className="text-purple-600">
                  {aiScanResults.recommendations}
                </p>
              </div>
            )}
