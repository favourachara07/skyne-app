...
type MetaBallsProps = {
  ...
  onTextTouch?: (touching: boolean) => void;
};
...
uniform vec4 iTextBounds; // x, y, width, height in screen coords
out vec4 outColor;
...
bool isInTextBounds(vec2 coord) {
    vec2 screenCoord = (coord / (iAnimationSize / iResolution.y)) + iResolution.xy * 0.5;
    return screenCoord.x >= iTextBounds.x && 
           screenCoord.x <= iTextBounds.x + iTextBounds.z &&
           screenCoord.y >= iResolution.y - iTextBounds.y - iTextBounds.w && 
           screenCoord.y <= iResolution.y - iTextBounds.y;
}
...
void main() {
    ...
    // Check if metaball is touching text area
    bool touchingText = isInTextBounds(coord) && f > 0.1;
...
const MetaBalls: React.FC<MetaBallsProps> = ({
  ...
  useEffect(() => {
    ...
    function updateTextBounds() {
      ...
      const relativeY = textRect.top - containerRect.top;
      program.uniforms.iTextBounds.value.set(
        relativeX * dpr,
        relativeY * dpr,
        textRect.width * dpr,
        textRect.height * dpr
    ...
    function update(t: number) {
      ...
      const touching = checkTextCollision();
      if (touching !== lastTouchState) {
        onTextTouch(touching);
  ...
  return     <div ref={containerRef} className="w-full h-full relative">
      ...
      >
        <h1 className="text-8xl md:text-9xl font-black tracking-wider select-none">
          SKYNE
        </h1>
      </div>
    </div>;
